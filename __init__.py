...
def train_model(model, optimizer, train_loader, label_x, label_y, epoch, lambda_super, train_name):
...
def muse_fit_predict(resultsdir, index, data_x,
    ...
    model = structured_embedding(feature_dim_x, feature_dim_y, latent_dim, n_hidden, dropout).to(device)
    ...
    for epoch in range(n_epochs_init):
        model.train()
        train_model(model, optimizer, train_loader, label_x, label_y, epoch, 0, 'init_recon')
    ...
    for epoch in range(n_epochs_init):
        model.train()
        train_model(model, optimizer, train_loader, label_x, label_y, epoch, lambda_super, 'init_both')
    ...
    label_x = transform(make_matrix_from_labels(update_label_x)).to(device)
    
    update_label_y, _, _ = phenograph.cluster(latent_y.detach().cpu().numpy(), k=k)
    label_y = transform(make_matrix_from_labels(update_label_y)).to(device)
    
    # TRAIN WITH UPDATED LABELS
    for epoch in range(n_epochs):
        model.train()
        train_model(model, optimizer, train_loader, label_x, label_y, epoch, lambda_super, 'train')
